...
export default function Planner() {
  const { token } = useAuth();
  const { allItineraries, refreshItineraries, weather, currentCoords, places } = useData();
  ...
  const saveSuggestionToToday = async (stops) => {
    try {
  ...
  const handleGenerateItinerary = async () => {
    ...
    try {
      ...
      const allPlaces = todaysPlans.flatMap((it) => it.places);
        ...
        <div>
              ...
              {generatedPlan && (
                ...
                >
                  ...
                  <p style={{ color: "#e2e8f0", fontSize: "0.9rem", marginBottom: "12px" }}>
                    ...
                    {generatedPlan.best_start_time && (
                      <>
                        {" "}
                        Suggested start: <strong>{generatedPlan.best_start_time}</strong>
                      </>
                    )}
                  </p>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                              ...
                              {item.activity_suggestion && (
                                <div style={{ fontStyle: "italic", marginTop: "2px" }}>{item.activity_suggestion}</div>
                              )}
                            </div>
                          </div>
                        </div>
                      ...
                      {generatedPlan.stops?.map((stop, idx) => (
                        ...
                        >
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                              {stop.category} · {stop.distance} km · {stop.travelMins} min
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  <div style={{ marginTop: "16px", display: "flex", gap: "12px" }}>
                    <button
                      onClick={() => saveSuggestionToToday(generatedPlan.stops)}
                      style={{
                        flex: 1,
                        padding: "10px",
                        background: "var(--accent-blue)",
          ...
          <div style={{ marginTop: "40px" }}>
            ...
            <div style={{ display: "flex", gap: "12px", marginBottom: "20px" }}>
              <input
                type="text"
                placeholder="e.g., Rainy afternoon with kids, or Romantic evening near Intramuros"
                value={aiPrompt}
